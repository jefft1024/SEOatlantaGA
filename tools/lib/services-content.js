/* Shared service-page content + renderer.
 *
 * Used by tools/build-services.js (static generation / defaults) AND by
 * api/service-page.js (live rendering with dashboard overrides). Keeping one
 * renderer guarantees an edited page is byte-identical to the built one except
 * for the words that changed.
 *
 * A page's editable content is a single flat object (see defaultContent). The
 * live renderer starts from these defaults and swaps in any overrides stored in
 * Supabase, so a page renders exactly as today until something is edited. */
...
const BY_SLUG = {};
SVC.forEach((s) => { BY_SLUG[s.slug] = s; });

/* The flat, editable content object a page renders from. */
function defaultContent(slug) {
  const s = BY_SLUG[slug];
  if (!s) return null;
  const seo = SEO[slug];
  const h1b = (s.h1[1].match(/>([^<]*)</) || [null, ""])[1];
  return {
    slug: s.slug,
    name: s.name,
    lower: s.lower,
    title: s.title,
    desc: s.desc,
    h1a: s.h1[0].replace(/\s+$/, ""),
    h1b: h1b,
    sub: s.sub,
    introHead: seo.introHead,
    intro: s.intro.slice(),
    h2Included: seo.h2Included,
    feats: s.feats.map((f) => f.slice()),
    h2Process: seo.h2Process,
    steps: s.steps.map((f) => f.slice()),
    h2ForWho: seo.h2ForWho,
    forWho: s.forWho.slice(),
    tail: seo.tail,
    h2Faq: seo.h2Faq,
    faqs: seo.faqs.concat(s.faqs).map((f) => f.slice())
  };
}

const REL = (slug) => SVC.filter((s) => s.slug !== slug).slice(0, 3);

/* Render the full HTML document for a service page from a content object.
 * `C` is tools/lib/chrome.js. Output is identical to the legacy builder. */
function renderServiceHTML(content, C) {
  const s = {
    slug: content.slug,
    name: content.name,
    lower: content.lower,
    title: content.title,
    desc: content.desc,
    h1: [`${content.h1a} `, `<span class="blue">${content.h1b}</span>`],
    sub: content.sub,
    intro: content.intro,
    feats: content.feats,
    steps: content.steps,
  ...
  const seo = {
    introHead: content.introHead,
    tail: content.tail,
    h2Included: content.h2Included,
    h2Process: content.h2Process,
    h2ForWho: content.h2ForWho,
    h2Faq: content.h2Faq
  };
...
module.exports = { SVC, ALT, SEO, defaultContent, renderServiceHTML, SLUGS: SVC.map((s) => s.slug) };
